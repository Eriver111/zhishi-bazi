...
function renderDayMasterPower(bazi){var c=document.getElementById('dayMasterPower');if(!c)return;var m={'甲':'木','乙':'木','丙':'火','丁':'火','戊':'土','己':'土','庚':'金','辛':'金','壬':'水','癸':'水'};var g=bazi.day.gan,w=m[g]||'?';var r;try{r=calcDayMasterStrength(bazi)}catch(e){r={score:50,level:'中和',detail:'日主中和'}}var l=r.score||50;var lb=r.level||'中和';var co=(lb==='极强'||lb==='偏强')?'#e44':lb==='中和'?'#ca4':'#48f';var h='<div style="display:flex;align-items:center;gap:8px;padding:4px 0">';h+='<span style="font-size:11px;color:var(--tx3)">身弱</span>';h+='<div style="flex:1;height:8px;background:rgba(255,255,255,.1);border-radius:4px"><div style="width:'+l+'%;height:100%;background:'+co+';border-radius:4px"></div></div>';h+='<span style="font-size:11px;color:var(--tx3)">身强</span>';h+='<span style="font-weight:700;color:'+co+';font-size:15px;margin-left:8px">'+lb+'</span></div>';h+='<p style="color:var(--tx2);font-size:11px;margin-top:4px;line-height:1.6">'+r.detail+'</p>';c.innerHTML=h}
function renderPattern(bazi){var c=document.getElementById('patternAnalysis');if(!c)return;var p;try{p=typeof BaZiCalculator!=='undefined'&&BaZiCalculator.getPattern?BaZiCalculator.getPattern(bazi):null}catch(e){p=null}if(!p||!p.name){c.innerHTML='<p style="color:var(--gold-l);font-size:15px;font-weight:700;margin-bottom:4px">格局不显</p><p style="color:var(--tx2);font-size:12px;line-height:1.6">需结合天干透出与地支合局综合判断。</p>';return}var source=p.source||('月令'+p.monthZhi+' · 五行'+p.monthWx);var broken=p.status==='破格'?'<p style="color:#a45b4f;font-size:10px;margin:2px 0 5px">破格'+(p.breakReasons&&p.breakReasons.length?' · '+p.breakReasons.join('；'):'')+'</p>':'';c.innerHTML='<p style="color:var(--gold-l);font-size:15px;font-weight:700;margin-bottom:4px">'+p.name+'</p>'+broken+'<p style="color:var(--tx2);font-size:12px;line-height:1.6">'+p.desc+'</p><p style="color:var(--tx3);font-size:10px;margin-top:2px">'+source+'</p>'}
function renderYongJi(bazi){var c=document.getElementById('yongJiAnalysis');if(!c)return;var yj;try{yj=typeof BaZiCalculator!=='undefined'&&BaZiCalculator.getYongJi?BaZiCalculator.getYongJi(bazi):null}catch(e){yj=null}if(!yj){c.innerHTML='<p style="color:var(--tx2);font-size:12px">喜用忌神数据暂不可用</p>';return}var wxColors={'木':'#6db86d','火':'#e07050','土':'#c9a84c','金':'#e8d5a3','水':'#5b9fd4'};var tag=function(wx,label,color){return'<span style="display:inline-block;padding:3px 12px;margin:2px;border-radius:12px;font-size:12px;font-weight:700;color:#fff;background:'+(wxColors[wx]||'#888')+'">'+wx+'</span>'};var h='';h+='<div style="margin-bottom:8px"><span style="font-size:12px;color:var(--tx3)">喜神</span><br>'+(yj.xiShen&&yj.xiShen.length?yj.xiShen.map(function(w){return tag(w,'喜神',wxColors[w])}).join(' '):'<span style="color:var(--tx3)">—</span>')+'</div>';h+='<div style="margin-bottom:8px"><span style="font-size:12px;color:var(--tx3)">用神</span><br>'+(yj.yongShen&&yj.yongShen.length?yj.yongShen.map(function(w){return tag(w,'用神',wxColors[w])}).join(' '):'<span style="color:var(--tx3)">—</span>')+'</div>';h+='<div style="margin-bottom:10px"><span style="font-size:12px;color:var(--tx3)">忌神</span><br>'+(yj.jiShen&&yj.jiShen.length?yj.jiShen.map(function(w){return tag(w,'忌神',wxColors[w])}).join(' '):'<span style="color:var(--tx3)">—</span>')+'</div>';h+='<p style="color:var(--tx2);font-size:11px;line-height:1.6;padding:8px;background:rgba(255,255,255,.02);border-radius:6px">'+yj.reasoning+'</p>';c.innerHTML=h}
