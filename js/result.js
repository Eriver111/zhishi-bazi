...
function renderElementRoleLedgerHtml(yj){
  ...
  var h='<div data-element-role-ledger="v2" data-fortune-guidance="v1" style="border:1px solid var(--bd);border-radius:10px;overflow:hidden">'+
  ...
  ['用神','喜神','忌神'].forEach(function(role,index){
    ...
    items.forEach(function(item){
      h+='<div style="display:flex;align-items:baseline;gap:7px;flex-wrap:wrap;'+(items.indexOf(item)?'margin-top:6px':'')+'"><span style="font-size:17px;font-weight:900;color:'+(colors[item.element]||'var(--gold-l)')+'">'+esc(item.element)+'</span>'+(item.useGodType?'<span style="font-size:9px;font-weight:700;color:var(--gold-l);padding:1px 6px;border:1px solid rgba(201,168,76,.28);border-radius:8px">'+esc(item.useGodType)+'</span>':'')+'<span style="font-size:10px;color:var(--tx2)">'+esc(item.fortuneDirection)+'</span><span style="margin-left:auto;font-size:9px;color:var(--tx3)">'+esc(item.fortuneLevel)+'</span></div>';
    });
    h+='</div></div></div>';
  });
