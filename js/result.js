...
function renderPattern(bazi){var c=document.getElementById('patternAnalysis');if(!c)return;var p;try{p=typeof BaZiCalculator!=='undefined'&&BaZiCalculator.getPattern?BaZiCalculator.getPattern(bazi):null}catch(e){p=null}if(!p||!p.name){c.innerHTML='<p style="color:var(--gold-l);font-size:15px;font-weight:700;margin-bottom:4px">格局不显</p><p style="color:var(--tx2);font-size:12px;line-height:1.6">需结合天干透出与地支合局综合判断。</p>';return}var subtype='';if(p.monthShiShenGan&&p.monthShiShenZhi)subtype='<p style="color:var(--tx3);font-size:10px;margin-top:2px">月干'+p.monthGan+'('+p.monthShiShenGan+') · 月支'+p.monthZhi+'('+p.monthShiShenZhi+') · 月令五行：'+p.monthWx+'</p>';c.innerHTML='<p style="color:var(--gold-l);font-size:15px;font-weight:700;margin-bottom:4px">'+p.name+'</p><p style="color:var(--tx2);font-size:12px;line-height:1.6">'+p.desc+'</p>'+subtype}
