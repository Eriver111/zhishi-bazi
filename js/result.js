    ...
    try {
      ...
      if(_bazi){
        ...
        if(typeof BaZiCalculator!=='undefined'){
          ...
          try{d.yongJi=BaZiCalculator.getYongJi(_bazi);d.pattern=d.yongJi&&d.yongJi.resolvedPattern?d.yongJi.resolvedPattern:BaZiCalculator.getPattern(_bazi)}catch(e){try{d.pattern=BaZiCalculator.getPattern(_bazi)}catch(_) {}}
        }
...
function renderPattern(bazi,facts){
  ...
  try{
    p=facts&&facts.pattern?facts.pattern:(typeof BaZiCalculator!=='undefined'&&BaZiCalculator.getPattern?BaZiCalculator.getPattern(bazi):null);
    yj=facts&&facts.yongJi?facts.yongJi:(typeof BaZiCalculator!=='undefined'&&BaZiCalculator.getYongJi?BaZiCalculator.getYongJi(bazi):null);
  }catch(e){p=null;yj=null}
  ...
  var broken=p.status==='破格'?'<p style="color:#a45b4f;font-size:10px;margin:2px 0 5px">破格'+(p.breakReasons&&p.breakReasons.length?' · '+p.breakReasons.join('；'):'')+'</p>':'';
  var related=(p.relatedPatterns||[]).map(function(x){var reasons=x.status==='条件待定'?x.pendingReasons:(x.status==='破格'?x.breakReasons:[]);var detail=reasons&&reasons.length?' · '+reasons.join('；'):'';return '<p style="color:var(--tx2);font-size:10px;line-height:1.55;margin-top:3px">兼见 '+x.name+'·'+x.status+detail+'</p>'}).join('');
  var following=yj&&yj.followingCandidate;
  var followingNote=following?'<div style="margin-top:8px;padding:8px 10px;border:1px solid rgba(201,168,76,.28);border-radius:8px;background:rgba(201,168,76,.06)"><p style="font-size:10px;font-weight:700;color:var(--gold-l);margin:0 0 3px">'+following.name+' · 尚未成立</p><p style="font-size:10px;line-height:1.55;color:var(--tx2);margin:0">'+following.desc+'</p></div>':'';
  var structureNote=p.formationRoute?'<div style="margin:6px 0;padding:7px 9px;border-left:2px solid var(--gold);background:rgba(201,168,76,.06);font-size:10px;line-height:1.65;color:var(--tx2)"><b style="color:var(--tx)">制化方式：</b>'+p.formationRoute+(p.structureResult?'<br><b style="color:var(--tx)">结构结果：</b>'+p.structureResult+' · '+(p.structureStatus||'待裁决'):'')+(p.structureBreakReasons&&p.structureBreakReasons.length?'<br><span style="color:var(--tx3)">'+p.structureBreakReasons.join('；')+'</span>':'')+'</div>':'';
  c.innerHTML='<p style="color:var(--gold-l);font-size:15px;font-weight:700;margin-bottom:4px">'+(p.displayName||p.name)+(p.congGe?'<span style="display:inline-block;font-size:9px;font-weight:700;color:#1a1408;background:#e8c05a;border-radius:8px;padding:1px 7px;margin-left:5px;vertical-align:2px">从格</span>':'')+'</p>'+broken+structureNote+'<p style="color:var(--tx2);font-size:12px;line-height:1.6">'+p.desc+'</p><p style="color:var(--tx3);font-size:10px;margin-top:2px">'+source+'</p>'+related+followingNote;
}
function renderElementRoleLedgerHtml(yj){
  ...
  var h='<div data-element-role-ledger="v3" data-fortune-guidance="v2" style="border:1px solid var(--bd);border-radius:10px;overflow:hidden">'+
    '<div style="padding:10px 12px;border-left:3px solid var(--gold);background:rgba(201,168,76,.08)">'+
...
function renderYongJi(bazi,facts){
  var c=document.getElementById('yongJiAnalysis');if(!c)return;var yj;
  try{yj=facts&&facts.yongJi?facts.yongJi:(typeof BaZiCalculator!=='undefined'&&BaZiCalculator.getYongJi?BaZiCalculator.getYongJi(bazi):null)}catch(e){yj=null}
  if(!yj){c.innerHTML='<p style="color:var(--tx2);font-size:12px">喜用忌神数据暂不可用</p>';return}
  if(yj.elementRoleLedger&&yj.elementRoleLedger.entries&&yj.elementRoleLedger.entries.length){c.innerHTML=renderElementRoleLedgerHtml(yj);return}
