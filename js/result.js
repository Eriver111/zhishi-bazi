...
function renderPattern(bazi,facts){
  ...
  var related=(p.relatedPatterns||[]).map(function(x){var detail=x.status==='破格'&&x.breakReasons&&x.breakReasons.length?' · '+x.breakReasons.join('；'):'';return '<p style="color:var(--tx2);font-size:10px;line-height:1.55;margin-top:3px">兼见 '+x.name+'·'+x.status+detail+'</p>'}).join('');
