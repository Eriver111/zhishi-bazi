...
function initHePanPaywall(p1,p2,relationType){
  ...
  pw.innerHTML='<h3 style="color:var(--gold-l);margin:8px 0">完整合盘分析报告</h3><p style="color:var(--tx2);font-size:13px;text-align:center;line-height:1.8">跨盘干支交互 · 核心相处模式<br>未来三年关键节点 · 宜忌指南</p><div style="font-size:30px;font-weight:900;color:var(--gold-l);margin:10px 0">¥13.9</div><button class="submit-btn" onclick="hstartPay()" style="max-width:280px">支付 ¥13.9 单独解锁报告</button><p style="color:var(--tx3);font-size:11px;margin-top:8px">一次付费 · 永久查看 · 支持下载</p><div style="margin-top:22px;padding:16px 20px;background:rgba(201,168,76,.1);border:1px dashed var(--bd2);border-radius:12px;text-align:center"><p style="color:var(--gold-l);font-size:15px;font-weight:700;margin-bottom:4px">试试知时AI解读合盘</p><p style="color:var(--gold);font-size:13px;margin-top:6px;cursor:pointer" onclick="if(window._aiOpen)window._aiOpen()">前2次免费 · 深度解读合盘缘分</p></div>';
  wrap.appendChild(pw);
  hautoRestore();
}
...
function hinjectQRModal(){
  ...
  m.innerHTML='<div style="background:var(--card,#0d1525);border:1px solid var(--bd);border-radius:16px;padding:28px 24px;text-align:center;max-width:360px;width:90%;position:relative"><button onclick="document.getElementById(\'hepanQrModal\').style.display=\'none\'" style="position:absolute;top:10px;right:14px;background:none;border:none;color:var(--tx2);font-size:22px;cursor:pointer">&times;</button><h3 style="color:var(--gold-l);margin-bottom:8px">扫码支付 ¥13.9</h3><a id="hepanMobileBtn" href="#" target="_self" style="display:none;margin:0 auto 12px;padding:12px 20px;background:#1677FF;color:#fff;font-size:15px;font-weight:600;letter-spacing:2px;border-radius:6px;text-decoration:none;max-width:260px"><svg class="ui-icon" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="6" y="2" width="12" height="20" rx="2"/><path d="M10 5h4m-2 13h.01"/></svg> 打开支付 ¥13.9</a><div id="hepanQrContainer" style="margin:12px auto;width:200px;height:200px;background:#fff;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:14px;color:#333">生成二维码中...</div><p style="color:var(--tx2);font-size:12px;margin:8px 0">付款后返回此页自动确认，请勿重复支付</p><p id="hepanQrStatus" style="color:var(--tx3);font-size:11px">等待支付...</p><button id="hepanQrRetry" class="submit-btn" style="max-width:260px;display:none;margin-top:8px" onclick="hstartPay()">重新支付</button><button class="submit-btn" style="max-width:260px;margin-top:6px;background:rgba(255,255,255,.05);color:var(--tx);border:1px solid var(--bd)" onclick="hmanualUnlock()">我已付款，刷新状态</button></div>';
  document.body.appendChild(m);
}

var _hepanTimer=null, _hepanCreating=false;

function hstartPay(){
  if(_hepanCreating)return;
  _hepanCreating=true;
  if(_hepanTimer)_hepanTimer.stop();
  var modal=document.getElementById('hepanQrModal');if(modal)modal.style.display='flex';
  var status=document.getElementById('hepanQrStatus');if(status)status.textContent='正在连接支付...';
  var oldButton=document.getElementById('hepanMobileBtn');if(oldButton){oldButton.style.display='none';oldButton.removeAttribute('href');}
  var oldQr=document.getElementById('hepanQrContainer');if(oldQr)oldQr.textContent='正在连接支付...';
  ...
  .then(function(r){
    ...
    if(d.error){
      if(status)status.textContent=d.error;if(retry)retry.style.display='block';return;
    }
    ...
    if(isM&&payment.payUrl){
      if(mobileBtn){mobileBtn.href=payment.payUrl;mobileBtn.textContent='打开'+PaymentFlow.methodLabel(d)+'收银台 ¥13.9';mobileBtn.style.display='block';}
      if(status)status.textContent='正在打开'+PaymentFlow.methodLabel(d)+'收银台...';
      var hepanC=document.getElementById('hepanQrContainer');
      if(hepanC)hepanC.textContent='请在收银台完成付款';
      ...
      if(status)status.textContent='请用'+PaymentFlow.methodLabel(d)+'扫码支付 ¥13.9（支付后自动解锁）';
    }
    hpoll(pending);
  }).catch(function(e){
    var msg=e.name==='AbortError'?'请求超时，请检查网络后重试':e.message||'网络错误，请重试';
    if(status)status.textContent=msg;
    var r=document.getElementById('hepanQrRetry');if(r)r.style.display='block';
  }).finally(function(){_hepanCreating=false;});
}

function hpoll(pending){
  if(!pending||pending.h!==_hepanHash)return;
  if(_hepanTimer)_hepanTimer.stop();
  _hepanTimer=PaymentFlow.watchOrder({
    orderId:pending.oid,expectedType:'hepan',
    isPaid:function(d){return pending.h===_hepanHash&&((pending.legacy&&d.report_key==='legacy'&&d.status==='paid')||(d.status==='paid'&&d.report_type==='hepan'&&d.report_key===pending.k));},
    onState:function(message){var status=document.getElementById('hepanQrStatus');if(status)status.textContent=message;},
    onPaid:function(){localStorage.removeItem('hepan_ord');var modal=document.getElementById('hepanQrModal');if(modal)modal.style.display='none';hunlock();}
  });
}
