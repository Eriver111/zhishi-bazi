# S04 · 实际发送给模型的完整 messages 数组（复刻 api/ai-chat.js callAI，mode=pro，history=[]）
...
出生：1973年1月1日 11时 性别：男

四柱排盘：
  年柱：壬子 [水] 天干十神：正官 地支十神：七杀 纳音：桑柘木 藏干：癸(七杀)
  月柱：壬子 [水] 天干十神：正官 地支十神：七杀 纳音：桑柘木 藏干：癸(七杀)
  日柱：丁酉 [火] 天干十神：日主 地支十神：偏财 纳音：山下火 藏干：辛(偏财)
  时柱：辛亥 [金] 天干十神：偏财 地支十神：正官 纳音：钗钏金 藏干：壬(正官)、甲(正印)
...
⚠从格判定：从杀格（弃命从杀）
  解读：日主弱极，官杀成势，不得不从。"弃命从杀，杀旺为贵。"喜财官杀顺势，忌印比破格。
  喜：水、金 忌：木、火
命局格局：从杀格（透干取格类） 月令五行：水
格局状态：成格
格局成立条件清单：
  ✅ 从格成立 —— 弃命从杀，日主弱极顺势而从。

格局解读：日主弱极，官杀成势，不得不从。"弃命从杀，杀旺为贵。"喜财官杀顺势，忌印比破格。
...
喜用忌神分析：
  ...
  判定证据：
    ...
    - 格局：弃命从杀（原局七杀格，既成从格，以从格论）
    - 根气/透干：水：透于年柱天干、月柱天干，见于年柱地支、月柱地支、时柱地支
  五行归类理由：
    - 用神·水：从格顺势：官杀制身，是本局首要的调节力量；透于年柱天干、月柱天干，见于年柱地支、月柱地支、时柱地支
    - 喜神·金：财星耗身，用于辅助用神并维持命局流通；透于时柱天干，见于日柱地支
    - 忌神·木：印星生身，会加重当前失衡或破坏已有制化；原局未直接出现
    - 忌神·火：比劫同类，会加重当前失衡或破坏已有制化；透于日柱天干
  用神真假评估：
    - 水：真用神——根深蒂固有力，如大树扎根，用神得力者一生层次高（根气得分8）
      根气详情：子本气根；子本气根；亥本气根；透干year/month
    - 金：偏真用神——有根但不够深固，如小树有根，中年后运势方能支撑（根气得分3）
      根气详情：酉本气根；透干hour

四柱关系事件（事实层枚举）：
  - 天干五合：year+day（水、火化木），涉月令/日支
  - 天干五合：month+day（水、火化木），涉月令/日支
...
宫位远近分析：
  提纲(月柱)：月柱壬子（提纲），官杀当令克身——压力重重，但若有制化反成权威。
  归息(时柱)：时柱辛亥（归息），晚岁财星——老来财运，但须身强。
  祖业(年柱)：年柱壬子（祖业），祖上官杀——家规严苛或祖上有权威传承。
...
日支（夫妻宫）专项分析：
  ...
  日主根气：长生（根气分2）
  稳定度：稳固——配偶宫根基扎实，感情稳定
  三合角色：酉为金局之一，待大运/流年补全
  三会角色：酉属金方，为三会局成员
  藏干详析：本气辛(偏财)——本气偏财——配偶慷慨、不拘小节
  综合：日支酉为偏财，长生。稳固——配偶宫根基扎实，感情稳定。酉为金局之一，待大运/流年补全
...
大运喜用忌联动分析：
  一生8步大运中，喜运2步，忌运5步。运途多有波折，宜守不宜攻。
  癸丑（3-1984岁）：天干水为用神，地支土为中性 → 综合判定：偏喜
    运程：大运癸丑（3-1984），天干水官杀（用神），地支土（中性）。此运中和，稳扎稳打。
  甲寅（13-1994岁）：天干木为忌神，地支木为忌神 → 综合判定：忌运
    运程：大运甲寅（13-1994），天干木印星（忌神），地支木（忌神）。此运忌神当道，宜守不宜攻。
  乙卯（23-2004岁）：天干木为忌神，地支木为忌神 → 综合判定：忌运 [大运冲日支（夫妻宫/自身根基）——十年动荡]
    运程：大运乙卯（23-2004），天干木印星（忌神），地支木（忌神）。此运忌神当道，宜守不宜攻。
  丙辰（33-2014岁）：天干火为忌神，地支土为中性 → 综合判定：偏忌
    运程：大运丙辰（33-2014），天干火比劫（忌神），地支土（中性）。此运中和，稳扎稳打。
  丁巳（43-2024岁）：天干火为忌神，地支火为忌神 → 综合判定：忌运
    运程：大运丁巳（43-2024），天干火比劫（忌神），地支火（忌神）。此运忌神当道，宜守不宜攻。
  戊午（53-2034岁）：天干土为中性，地支火为忌神 → 综合判定：偏忌 [大运冲提纲（月支子）——十年根基动摇]
    运程：大运戊午（53-2034），天干土食伤（中性），地支火（忌神）。此运中和，稳扎稳打。
  己未（63-2044岁）：天干土为中性，地支土为中性 → 综合判定：中性
    运程：大运己未（63-2044），天干土食伤（中性），地支土（中性）。此运中和，稳扎稳打。
  庚申（73-2054岁）：天干金为喜神，地支金为喜神 → 综合判定：喜运
    运程：大运庚申（73-2054），天干金财星（喜神），地支金（喜神）。此运喜用双全，人生上升期。

神煞：天乙贵人(ji-shen)、文昌贵人(ji-shen)、太极贵人(ji-shen)、月德贵人(ji-shen)、福星贵人(ji-shen)、学堂(ji-shen)、桃花(zhong)、将星(ji-shen)、天喜(ji-shen)、亡神(ji)、血刃(ji)、月德合(ji-shen)、披麻(ji)

当前时间：2026年
当前大运：戊午（伤官）2025-2034年 53岁
当前流年：丙午（劫财）2026年
  注意：当前行伤官大运，遇劫财流年——需结合大运流年与原局关系综合判断吉凶。
...
天干五合：
  年柱壬 + 日柱丁 → 争合不化（化木）：丁被壬争合——情不专一，化气不成
  月柱壬 + 日柱丁 → 争合不化（化木）：丁被壬争合——情不专一，化气不成
...
```
【排盘事实锁】日主旺衰=「极弱（1）」，候选格局=「从杀格」，状态=「成格」。以上字段不另行重算；破格不得写成已成格；旺衰档位、格局状态、risk severity 均为冻结标签，禁止近义词换级。
```
