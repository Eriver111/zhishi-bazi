...
喜用忌神分析：
  ...
  判定证据：
    ...
    - 候选对比：取木（印，综合分 30.0）；未取火（比劫，30.0）、水（官杀，-24.0）、土（食伤，-30.0）、金（财，-30.0）
  五行归类理由：
...
日支（夫妻宫）专项分析：
  ...
  综合：日支亥为正官，无根。稳固——配偶宫根基扎实，感情稳定。亥参与木局（未卯），日支有合局之助
...
```
【排盘事实锁】日主旺衰=「极弱（20）」，候选格局=「偏财格」，状态=「破格」，原因=「月令受时柱冲；日主极弱，难以承载格局用神」。以上字段不另行重算；破格不得写成已成格；旺衰档位、格局状态、risk severity 均为冻结标签，禁止近义词换级。
```
