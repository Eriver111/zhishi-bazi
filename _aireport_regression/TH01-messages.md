...
喜用忌神分析：
  ...
  判定证据：
    ...
    - 候选对比：取土（财，综合分 35.0）；未取火（食伤，37.0）、金（官杀，25.0）、木（比劫，-19.0）、水（印，-25.0）
  五行归类理由：
...
```
【排盘事实锁】日主旺衰=「偏强（75）」，候选格局=「正印格」，状态=「破格」，原因=「月令受日柱冲」。以上字段不另行重算；破格不得写成已成格；旺衰档位、格局状态、risk severity 均为冻结标签，禁止近义词换级。
```
