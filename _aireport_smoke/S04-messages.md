...
日支（夫妻宫）专项分析：
  ...
  综合：日支酉为偏财，长生。稳固——配偶宫根基扎实，感情稳定。无冲合刑害。；酉为金局之一，待大运/流年补全
...
大运喜用忌联动分析：
  ...
  癸丑（7-2048岁）：天干水为用神，地支土为中性 → 综合判定：偏喜
    运程：大运癸丑（7-2048），天干水官杀（用神），地支土（中性）。此运中和，稳扎稳打。
  甲寅（17-2058岁）：天干木为忌神，地支木为忌神 → 综合判定：忌运
    运程：大运甲寅（17-2058），天干木印星（忌神），地支木（忌神）。此运忌神当道，宜守不宜攻。
  乙卯（27-2068岁）：天干木为忌神，地支木为忌神 → 综合判定：忌运 [大运冲日支（夫妻宫/自身根基）——十年动荡]
    运程：大运乙卯（27-2068），天干木印星（忌神），地支木（忌神）。此运忌神当道，宜守不宜攻。
  丙辰（37-2078岁）：天干火为忌神，地支土为中性 → 综合判定：偏忌
    运程：大运丙辰（37-2078），天干火比劫（忌神），地支土（中性）。此运中和，稳扎稳打。
  丁巳（47-2088岁）：天干火为忌神，地支火为忌神 → 综合判定：忌运
    运程：大运丁巳（47-2088），天干火比劫（忌神），地支火（忌神）。此运忌神当道，宜守不宜攻。
  戊午（57-2098岁）：天干土为中性，地支火为忌神 → 综合判定：偏忌 [大运冲提纲（月支子）——十年根基动摇]
    运程：大运戊午（57-2098），天干土食伤（中性），地支火（忌神）。此运中和，稳扎稳打。
  己未（67-2108岁）：天干土为中性，地支土为中性 → 综合判定：中性
    运程：大运己未（67-2108），天干土食伤（中性），地支土（中性）。此运中和，稳扎稳打。
  庚申（77-2118岁）：天干金为喜神，地支金为喜神 → 综合判定：喜运
    运程：大运庚申（77-2118），天干金财星（喜神），地支金（喜神）。此运喜用双全，人生上升期。
...
```
【排盘事实锁】日主旺衰=「极弱（1）」，候选格局=「从杀格」，状态=「成格」。以上字段不另行重算；破格不得写成已成格。
```
