# 个人排盘·深度报告 判定逻辑（js/bazi.js 全量源码）

> 2026-08-15 提取，供 GPT 审阅。文件 js/bazi.js 共 5616 行，本文档按函数为单位完整收录判定链。
> 冻结声明：标注 ★ 的函数属于 `CORE_ENGINE_FROZEN_AFTER_P4A` 冻结面（旺衰/取格/从格/用神/喜忌/候选评分）——
> 按冻结纪律，除非新多盘同机制明确错误，不得修改。其余函数为报告解释层/历法层，可正常审阅修复。

## 函数索引（按源文件行号）

| 函数 | 行号 | 职责 |
|---|---|---|
| getJieQiDatesLegacy | 95-214 | 节气时刻（近似算法，历法基础） |
| getJieQiDates | 215-281 | 节气时刻（太阳视黄经 NOAA/Meeus 低阶，历法基础） |
| getYearPillar | 282-316 | 年柱判定（立春分界） |
| getMonthPillar | 317-404 | 月柱判定（节气分界 + 五虎遁） |
| getDayPillar | 405-428 | 日柱推算（公历 → 干支） |
| getHourPillar | 429-462 | 时柱判定（五鼠遁） |
| getShiShen | 463-479 | 十神映射（日干对目标干） |
| getWuXingRelation | 480-501 | 五行生克关系 |
| calculateShiShen | 502-516 | 十神计算（生克 + 阴阳同性异性） |
| getNaYin | 517-528 | 纳音 |
| getCangGan | 529-549 | 地支藏干 |
| calculateBaZi | 550-644 | 排盘总装（四柱 + 藏干 + 十神 + 纳音） |
| normalizeBirthInput | 645-693 | 出生输入归一化 |
| calculateFromBirthInput | 694-707 | 从输入参数起盘 |
| buildBaZiFromPillars | 708-761 | 从已知四柱构造命盘（合盘/测试用） |
| countWuXing | 762-812 | 五行统计 |
| calculateDaYun | 813-926 | 大运排定（起运岁数 + 顺逆排运） |
| birthDateToDecimal | 927-935 | 日期换算为十进制 |
| calculateLiuNian | 936-1562 | 流年排定 |
| calculateShenSha | 1563-1678 | 神煞计算 |
| calculateChengGu | 1679-1831 | 称骨 |
| analyzePei | 1832-1880 | 配偶分析 |
| calculateSpouseAge | 1881-2104 | 配偶年龄推算 |
| getDaysFromJieQi | 2105-2124 | 距节气天数（人元司令基础） |
| getRenYuanSiLing | 2125-2140 | 人元司令（司权天数） |
| getRenYuanEvidence | 2141-2177 | 人元司令证据 |
| calcDayMasterStrength | 2178-2762 | 日主旺衰判定（★ 冻结层 CORE_ENGINE_FROZEN_AFTER_P4A） |
| analyzeParents | 2763-3069 | 六亲·父母分析 |
| analyzeHourYearCharacter | 3070-3089 | 年/月/时柱性格 |
| analyzeCharacter | 3090-3208 | 性格分析 |
| analyzeWealth | 3209-3407 | 财富分析 |
| classifyFortuneElement | 3408-3422 | 事业五行分类 |
| analyzeFortune | 3423-3628 | 事业运势分析 |
| analyzeThisYear | 3629-3773 | 流年应期分析（用神/喜神/忌神触发） |
| analyzeStudy | 3774-3979 | 学业分析 |
| getTrueSolarHour | 3980-4091 | 真太阳时（省份经度修正） |
| finalizePatternStatus | 4092-4208 | 格局状态收口（成格/破格 + 救应） |
| getPattern | 4209-4397 | 取格判定（★ 冻结层，P4-A 已修 A1/A2） |
| normalizeYongJiLists | 4398-4411 | 用神列表归一化 |
| finalizeYongJiResult | 4412-4530 | 用神收口（扶抑/调候/救应 method + 理由链） |
| evaluateYongShenQuality | 4531-4629 | 用神质量评估（根气/受克/被合绊） |
| calcCandidateScores | 4630-4864 | 候选用神评分（★ 冻结层 P1） |
| getYongJi | 4865-4990 | 用神判定总装（★ 冻结层） |
| getPillarRelations | 4991-5032 | 天干关系（生克/合） |
| getBranchRelations | 5033-5089 | 地支关系（冲刑害合） |
| getChangSheng | 5090-5120 | 十二长生表 |
| getCongGe | 5121-5238 | 从格判定（★ 冻结层，P4-A 已修覆盖优先级） |
| getGanHe | 5239-5327 | 天干合化 |
| getSanHui | 5328-5359 | 三会方 |
| getCangGanDepth | 5360-5386 | 藏干深度 |
| analyzeDayBranch | 5387-5497 | 日支分析 |
| getProfessionalReportFacts | 5498-5617 | ★ 深度报告事实总装（报告内容判定链的收口函数） |

## 源码（按源文件顺序）

### getJieQiDatesLegacy（行 95-214）
...
### getJieQiDates（行 215-281）

> 节气时刻（太阳视黄经 NOAA/Meeus 低阶，历法基础）
...
### calculateFromBirthInput（行 694-707）
...
### buildBaZiFromPillars（行 708-761）
...
### calculateSpouseAge（行 1881-2104）
...
### getRenYuanEvidence（行 2141-2177）
...
### calcDayMasterStrength（行 2178-2762）

> 日主旺衰判定（★ 冻结层 CORE_ENGINE_FROZEN_AFTER_P4A）
...
### analyzeHourYearCharacter（行 3070-3089）
...
### classifyFortuneElement（行 3408-3422）
...
### finalizePatternStatus（行 4092-4208）
...
### getPattern（行 4209-4397）

> 取格判定（★ 冻结层，P4-A 已修 A1/A2）
...
### normalizeYongJiLists（行 4398-4411）
...
### finalizeYongJiResult（行 4412-4530）

> 用神收口（扶抑/调候/救应 method + 理由链）
...
### evaluateYongShenQuality（行 4531-4629）
...
### calcCandidateScores（行 4630-4864）
...
### getPillarRelations（行 4991-5032）
...
### getBranchRelations（行 5033-5089）
...
### getCongGe（行 5121-5238）

> 从格判定（★ 冻结层，P4-A 已修覆盖优先级）
...
### getProfessionalReportFacts（行 5498-5617）

> ★ 深度报告事实总装（报告内容判定链的收口函数）
