...
test('shared light theme suppresses dynamic background canvas while the homepage restores its restrained canvas', () => {
  ...
  for (const page of publicPages) {
    ...
    assert.ok(hrefs.includes('css/theme-light.css?v=5'), `${page} must load the cache-busted shared light theme`);
  }
  const homeLinks = stylesheetLinks(activeMarkup(read('index.html'))).map(({ 0: tag }) => attributeValue(tag, 'href'));
  assert.ok(homeLinks.indexOf('css/theme-light-home.css?v=4') > homeLinks.indexOf('css/theme-light.css?v=5'));
...
test('homepage loads a dedicated responsive light stylesheet after the shared theme', () => {
  ...
  assert.ok(links.indexOf('css/theme-light-home.css?v=4') > links.indexOf('css/theme-light.css?v=5'));
