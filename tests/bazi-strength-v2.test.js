...
test('护栏盘：失令+单弱根/无强根盘分数档位零漂移', () => {
  const cases = [
    ...
    // 卯中乙木不是癸水根，撤销原错误的杀印补偿6分。
    { gz: ['庚申', '己丑', '癸卯', '丁巳'], score: 26, level: '极弱' },
  ];
...
test('禄旺根被原局六冲时不得按完整多重强根加分', () => {
  const cases = [
    ...
    // 辰中乙印未透且被子辰半合牵制，通关补偿从13收为3；受冲午根不补回。
    { gz: ['丙午', '庚子', '丙辰', '丁巳'], score: 31, level: '偏弱' },
...
test('刑害落到禄旺根本身时不启用高额双根补偿', () => {
  const cases = [
    ...
    // 年月寅巳刑害共指同一甲木根，统一结算后不再重复扣 1 分。
    { gz: ['庚寅', '辛巳', '乙亥', '己卯'], score: 40, level: '中和' },
