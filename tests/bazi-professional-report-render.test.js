...
test('专业报告从一份事实对象渲染五个核心证据区域', () => {
  ...
  assert.match(rendered.nodes.xiyongAnalysis.innerHTML, /原局作用/);
  assert.match(rendered.nodes.xiyongAnalysis.innerHTML, /行运复核/);
  assert.match(rendered.nodes.xiyongAnalysis.innerHTML, /查看原局依据与具体条件/);
  ...
  assert.match(rendered.nodes.fortuneInteractionAnalysis.innerHTML, /原局基础方向/);
  assert.match(rendered.nodes.fortuneInteractionAnalysis.innerHTML, /本年验证/);
  assert.match(rendered.nodes.fortuneInteractionAnalysis.innerHTML, /十神只说明事项类型|十神名称仅作事项解释/);
});
