...
test('home and paipan share one versioned mobile shell without touching desktop markup', () => {
  ...
  for (const source of [home, paipan]) {
    ...
    assert.match(source, /js\/mobile-app-shell\.js\?v=11/);
  }
  ...
  assert.match(paipan, /<script src="js\/main\.js\?v=\d+"><\/script>\s*<script src="js\/mobile-app-shell\.js\?v=11"><\/script>/);
});
...
test('feature pages share the same back header and result uses a four-pillar mobile grid', () => {
  ...
  assert.match(auth, /mobile-app-shell\.js\?v=11/);
  assert.match(read('result.html'), /js\/auth\.js\?v=31/);
  ...
  assert.match(css, /0 6px 18px rgba\(45, 38, 31, \.20\)/);
  assert.match(css, /body\.mobile-page-result \.section-sizhu \*,[\s\S]*animation:\s*none !important;[\s\S]*transition:\s*none !important/);
  assert.match(css, /body\.mobile-page-result \.section-sizhu \.pp-col::before,[\s\S]*content:\s*none !important;[\s\S]*display:\s*none !important/);
