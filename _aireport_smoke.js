// AI 报告验证 · 3 盘冒烟（2026-08-14）：S04 / TH07 / R01
...
//  ③ 插入测试兑换码 AISMOKE01（credits=3, channel=qa）→ 真实 POST /api/ai-chat（mode=pro）
//  ④ 每盘后查 supabase chat_history 验证报告保存（user/assistant 行、内容一致）
//  ⑤ 验证扣减 3→2→1→0，最后删除测试码（chat_history 证据行保留并报告）
...
(async function () {
  ...
  // 检索 1900-2048 全部匹配年，优先 1940-2010（在世成年人）→ 1900-1939（老年/先人）→ 2011+（未来，最后手段）。
  // 未来日期会与 SYSTEM_PROMPT"现在是2026年"冲突，污染 AI 忠实度判定，故仅作最后兜底。
  function findBirth(gz, gender) {
    var hourIdx = ZHI.indexOf(gz[3][1]);
    var matches = [];
    for (var y = 1900; y <= 2048; y++) {
      var yp = ENG.calculate(y, 6, 15, 6, gender, 0, 0).year;
      if (yp.gan + yp.zhi !== gz[0]) continue;
      for (var m = 1; m <= 12; m++) {
        for (var d = 1; d <= 28; d++) {
    ...
    matches.sort(function (a, b2) { return rank(a) - rank(b2); });
  ...
  var { error: insErr } = await db.from('user_credits').upsert({
    code: TEST_CODE, order_id: 'qa-smoke-20260814', credits: 3, total_used: 0,
    created_at: nowIso, updated_at: nowIso, channel: 'qa'
  }, { onConflict: 'code' });
  if (insErr) throw new Error('插入测试码失败: ' + insErr.message);
  console.log('✅ 测试兑换码 ' + TEST_CODE + '（credits=3）已插入');
  ...
  // ---- 汇总 ----
  var sum = '# AI 报告冒烟汇总（2026-08-14）\n\n';
  sum += '> 线上真实调用 zhishi.online /api/ai-chat（生产 DeepSeek，model 由服务端钉死），mode=pro，新会话首问（history=[]）。\n';
  sum += '> 测试兑换码 AISMOKE01（qa 专用，3 次额度），调用完成后已删除；chat_history 证据行保留（code=AISMOKE01）。\n';
  sum += '> 性别：冒烟统一男命（乾造）。盲测盘未定义性别，大运顺逆按乾造计算。\n';
  ...
  console.log('\n✅ 汇总已落盘 _aireport_smoke/00-summary.md');
})().catch(function (e) {
