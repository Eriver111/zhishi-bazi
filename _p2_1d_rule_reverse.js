// P2.1 第三轮·资格反推验证（2026-08-13，GPT 第三轮裁决第1-3节）
// 1) Q9 已裁：午未合土为正确口径（bazi.js:2416 午未合火 = 数据错误）。本轮不改引擎，
//    探针侧统一按经典口径（午未合土）重算 18 盘事件，输出 H06/H18 反事实。
// 2) GPT 独立标签 → 最小有序规则集反推：S(月令支持档)×T(透干档)×D(破坏降级)，输出混淆矩阵。
// 3) 冲突盘交回 GPT；禁止盘号特判（脚本断言冲突集合，防止规则被静默调成 18/18）。
// 引擎 js/bazi.js 零改动；仅观测补丁（1c 同款）暴露引擎 dayBranchAdj 对照。
// 用法: node _p2_1d_rule_reverse.js
// 产物: _p2_1d_rule_reverse.csv
...
// 六合表：本轮统一经典口径（午未合土）。引擎评分层 ZHI_HE 的午未合火仅作旧行为记录对照。
...
// —— 18 盘样本（同 _p2_1c；expAdj = 旧引擎（午未合火口径）实测 dayBranchAdj，仅作旧行为记录）——
var CHARTS = [
...
var CHARTS_BY_ID = {};
CHARTS.forEach(function(c){ CHARTS_BY_ID[c.id] = c; });

// —— GPT 独立资格标签（2026-08-13 第三轮裁决，硬编码，H06/H18 按午未合土口径）——
var GPT_LABELS = {
  'H01':'真化','H02':'真化','H03':'真化','H07':'真化','H10':'真化','H13':'真化','H14':'真化',
  'H04':'趋化','H05':'趋化','H08':'趋化','H09':'趋化','H11':'趋化','H12':'趋化','H16':'趋化','H17':'趋化','H18':'趋化',
  'H06':'合而不化','H15':'合而不化'
};

// —— 合法性断言（60甲子奇偶 + 五虎遁 + 五鼠遁，同 1c）——
...
// —— 经典口径 dayBranchAdj 反事实复算（镜像引擎规则）——
...
// —— 引擎装载（观测补丁同 1c，仅暴露 dayBranchAdj 对照旧行为）——
function loadEngine(instrumented) {
  var src = baseCode;
  if (instrumented) {
    src = src.replace(/score \+= dayBranchAdj;\r?\n/,
      'score += dayBranchAdj;\n      global.__p2_1d_adj = dayBranchAdj;\n      // CF-P2.1D-INSTRUMENT：仅观测，不改变任何计分\n');
    if (src.indexOf('CF-P2.1D-INSTRUMENT') < 0) throw new Error('观测补丁未生效（replace 未匹配）');
  }
  var stitched = '';
...
// —— 最小有序规则集（GPT 第三轮裁决骨架，无盘号特判）——
// S1 强支持：月支五行===化神；S2 次级：化神旺/相；S0 无：休囚死
// T2 近透：月/时干透化神；T1 远透：仅年干透；T0 无透
// D1 成员遭直接六冲 → 真化降一级；D2 月令受冲 → 真化降一级；刑/害仅记录
...
CHARTS.forEach(function(c) {
  ...
  global.__p2_1d_adj = null;
  obsEng.dm(toBazi(gz));
  var engineAdj = global.__p2_1d_adj === null ? 0 : global.__p2_1d_adj;
  ...
  // 非午未盘：经典口径必须与引擎口径一致（午未合只影响 H06/H18）
...
// —— H06/H18 反事实专表（GPT 裁决第1节要求）——
console.log('========== 一、午未合土口径 H06/H18 反事实（引擎零改动）==========');
['H06','H18'].forEach(function(id) {
  var r = rows.filter(function(x){ return x.id === id; })[0];
  console.log('\n【' + id + '】' + r.gz);
  console.log('  六合对: ' + (id === 'H06' ? 'month-day(午未合土)' : 'month-day(午未合土)'));
  console.log('  化神: 土（经典口径）；旧引擎口径化火已废弃（bazi.js:2416 数据错误，Q9 已裁，暂不改引擎）');
  console.log('  月令支持: ' + id + ' 月支=' + r.mwx + '，化神土在' + id + '月为' + r.status + '（' + r.S + '）');
  console.log('  透干位置(土): ' + r.tou);
  console.log('  R1冲: ' + r.r1 + '   R2月令冲: ' + r.r2 + '   刑: ' + r.xing + '   害: ' + r.hai);
  console.log('  dayBranchAdj 反事实: 经典口径=' + (r.classicalAdj > 0 ? '+' : '') + r.classicalAdj +
    '（旧引擎口径实测=' + (r.engineAdj > 0 ? '+' : '') + r.engineAdj + '，仅作旧行为记录，不再作为命理基准）');
});
console.log('\n断言：H06 经典口径 0 / H18 经典口径 +1 已钉死 ✓');

// —— 反推结果总表 ——
console.log('\n========== 二、18盘 GPT标签 vs 规则预测 ==========');
console.log('GPT标签 / 新规则预测 / 是否一致 / 证据链');
rows.forEach(function(r) {
  console.log('  ' + r.id + '  ' + r.gpt + ' / ' + r.predLabel + ' / ' + (r.match ? '一致 ✓' : '✗不一致') + ' / ' + r.chain);
});

// —— 混淆矩阵 ——
var CATS = ['真化','趋化','合而不化'];
var matrix = {}; CATS.forEach(function(a){ matrix[a] = {}; CATS.forEach(function(b){ matrix[a][b] = 0; }); });
rows.forEach(function(r){ matrix[r.gpt][r.predLabel]++; });
console.log('\n========== 三、混淆矩阵（行=GPT标签，列=规则预测）==========');
console.log('            真化  趋化  合而不化');
CATS.forEach(function(a) {
  var line = '  ' + a + '      ';
  CATS.forEach(function(b) { line += '  ' + matrix[a][b]; });
  console.log(line);
});
var total = rows.length, agree = rows.filter(function(r){ return r.match; }).length;
console.log('一致率: ' + agree + '/' + total + '（' + Math.round(agree / total * 1000) / 10 + '%）');

// —— 冲突盘交回（断言：恰好 H06/H18 两盘，禁止静默调成 18/18）——
var conflicts = rows.filter(function(r){ return !r.match; });
if (conflicts.length !== 2 || conflicts[0].id !== 'H06' || conflicts[1].id !== 'H18') {
  throw new Error('冲突盘集合与预期不符（应恰为 H06+H18）：' + conflicts.map(function(x){ return x.id; }).join('、'));
}
console.log('\n========== 四、冲突盘（交回 GPT，不加盘号特判）==========');
conflicts.forEach(function(r) {
  console.log('  ' + r.id + '：GPT=' + r.gpt + '，规则=' + r.predLabel + '，证据链=' + r.chain);
});

// —— CSV 输出 ——
function csvQ(s) { return '"' + String(s).replace(/"/g, '""') + '"'; }
var header = ['编号','四柱','化神(经典口径)','月支五行','月令状态(旺相休囚死)','支持档','透干位置','透干档','R1冲','R2月令冲','刑(仅记录)','害(仅记录)','六合事件数','引擎口径adj(旧行为)','经典口径adj(反事实)','GPT标签','规则预测','是否一致','证据链'];
var csv = [header.join(',')];
rows.forEach(function(r) {
...
fs.writeFileSync(path.join(ROOT, '_p2_1d_rule_reverse.csv'), '﻿' + csv.join('\n'), 'utf-8');
console.log('\n已写入 _p2_1d_rule_reverse.csv（' + rows.length + ' 行）');
