...
// 接受浮点数（clock/minute 经真太阳时调整后可能为小数）
function num(value, name, fallback) {
  if ((value === '' || value === null || value === undefined) && fallback !== undefined) return fallback;
  const parsed = Number(value);
...
function normalizeBaziReportParams(raw) {
  ...
  const normalizedMode = text(raw.mode);
  const mode = normalizedMode === 'pillars' || text(raw.cal) === 'pillars'
    ? 'pillars'
    : text(raw.cal || (normalizedMode === 'lunar' ? 'lunar' : 'solar'));
  ...
  const result = {
    ...
    minute: num(raw.minute, 'minute', 0),
    clock: num(raw.clock, 'clock', num(raw.hour, 'hour')),
    gender,
    ziHourRule: text(raw.ziHourRule || raw.zi_hour_rule || (text(raw.zishi) === '1' ? 'next-day' : 'same-day')),
    trueSolarTime: text(raw.trueSolarTime) === 'disabled' || text(raw.solar) === '0' ? 'disabled' : 'enabled'
  };
