...
function getClientIp(req) {
  ...
  const forwarded = String(headers['x-forwarded-for'] || '').split(',').map(function (value) {
    return value.trim();
  }).filter(Boolean);
  if (forwarded.length) return forwarded[forwarded.length - 1].slice(0, 80);
  return String((req && req.socket && req.socket.remoteAddress) || (req && req.connection && req.connection.remoteAddress) || 'unknown').slice(0, 80);
}
...
function rateLimit(key, max, windowMs) {
  ...
  if (!entry || now - entry.start > windowMs) {
    _rl[key] = { start: now, count: 1, windowMs: windowMs };
    return true; // 允许
  }
  entry.count++;
...
setInterval(function () {
  var now = Date.now();
  for (var k in _rl) {
    if (now - _rl[k].start > (_rl[k].windowMs || 600000)) delete _rl[k];
  }
}, 600000).unref();

module.exports = { signToken, verifyToken, hashPassword, verifyPassword, requireAuth, rateLimit, getClientIp };
