...
async function getChartCalibrationSummary(userId, chartKey) {
  ...
  var eventLines = profile.events.filter(function(event) { return event.answer === 'yes' || event.answer === 'no'; }).slice(0, 20).map(function(event) {
    ...
    var statement = picked ? picked.label + (picked.detail ? '，具体是' + picked.detail : '') : (rejectedOptions.length ? '本题全部候选均不符合：' + rejectedOptions.map(function(option) { return option.label; }).join('、') : event.prompt);
    var mechanism = picked && picked.mechanismKey ? '；对应机制=' + picked.mechanismKey : (rejectedOptions.length ? '；已否认机制=' + rejectedOptions.map(function(option) { return option.mechanism_key || (option.domain + ':general'); }).join(',') : '');
