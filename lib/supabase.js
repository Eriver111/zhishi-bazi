...
async function getChartCalibrationSummary(userId, chartKey) {
  ...
  var eventLines = result.events.filter(function(event) { return event.answer === 'yes' || event.answer === 'no'; }).slice(0, 20).map(function(event) {
    ...
    var statement = picked ? picked.label + (picked.detail ? '，具体是' + picked.detail : '') : event.prompt;
    var mechanism = picked && picked.mechanismKey ? '；对应机制=' + picked.mechanismKey : '';
    return year + '年【' + (domainNames[domain] || '经历') + '】' + state + '：' + statement + mechanism + (event.note ? '；用户补充：' + event.note : '');
  });
  return [profileLine, deniedLine].concat(eventLines).filter(Boolean).join('\n');
}
