...
function normalizeCalibrationCandidates(candidates) {
  ...
  return candidates.slice(0, 24).map(function(item, index) {
    ...
    return {
      ...
      event_year: year, domain: domain, prompt: prompt, evidence: evidence,
...
async function initializeChartCalibration(userId, chartKey, chartSignature, candidates) {
  ...
  var calibration = {
    ...
    chart_signature: chartSignature, candidate_version: 'bazi-cal-v1', created_at: now, updated_at: now
  };
  var events = candidates.map(function(item) {
    return Object.assign({
      id: crypto.randomUUID(), calibration_id: calibration.id, user_id: userId,
      answer: null, actual_year: null, note: '', answered_at: null, created_at: now, updated_at: now
    }, item);
  });
  var memoryKey = userId + ':' + chartKey;
  var db = getSupabase();
...
async function answerChartCalibrationEvent(userId, chartKey, eventKey, answer, actualYear, note) {
  chartKey = normalizeChartKey(chartKey);
  eventKey = String(eventKey || '').replace(/[^a-zA-Z0-9:_-]/g, '').slice(0, 80);
  if (['yes', 'no', 'unsure'].indexOf(answer) < 0 || !eventKey) return null;
  actualYear = actualYear === null || actualYear === undefined || actualYear === '' ? null : parseInt(actualYear, 10);
  if (actualYear !== null && (!Number.isInteger(actualYear) || actualYear < 1900 || actualYear > 2200)) actualYear = null;
  ...
  var patch = { answer: answer, actual_year: actualYear, note: note, answered_at: new Date().toISOString(), updated_at: new Date().toISOString() };
  var db = getSupabase();
...
async function getChartCalibrationSummary(userId, chartKey) {
  ...
  return result.events.filter(function(event) { return event.answer === 'yes' || event.answer === 'no'; }).slice(0, 20).map(function(event) {
    var year = event.actual_year || event.event_year;
    var state = event.answer === 'yes' ? '用户确认发生' : '用户确认没有发生';
    return year + '年【' + (domainNames[event.domain] || '经历') + '】' + state + '：' + event.prompt + (event.note ? '；用户补充：' + event.note : '');
  }).join('\n');
}
