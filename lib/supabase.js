...
const {
  ...
  safeKey: safeCalibrationKey
} = require('./calibration-model.js');
...
async function getPaidReportAccess(userId, reportType, reportKey) {
  ...
  const earliestPaid = function(rows) {
    return (rows || []).filter(function(row) { return row && row.status === 'paid'; })
      .sort(function(a, b) {
        return String(a.paid_at || '9999').localeCompare(String(b.paid_at || '9999')) ||
  ...
  const db = getSupabase();
  if (!db) {
    const row = earliestPaid(Object.values(memStore.reportOrders).filter(function(row) {
  ...
  try {
    const { data, error } = await db.from('report_orders').select('order_id,paid_at,created_at')
      .eq('user_id', userId).eq('report_type', reportType)
      .eq('report_key', reportKey).eq('status', 'paid')
      .order('paid_at', { ascending:true }).order('created_at', { ascending:true })
      .order('order_id', { ascending:true }).limit(1);
    if (error) throw error;
...
async function getUserChatHistory(userId, limit) {
  ...
  const db = getSupabase();
  if (!db) {
    return memStore.chatHistory.filter(function(row) { return row.user_id === userId; }).slice(-lim);
  }
...
function normalizeChatMode(mode) {
  mode = String(mode || 'bazi').toLowerCase();
  return ['bazi', 'ziwei', 'hepan', 'liuren'].indexOf(mode) >= 0 ? mode : 'bazi';
}

function normalizeChartKey(chartKey) {
  var key = String(chartKey || 'general').replace(/[^a-zA-Z0-9:_-]/g, '').slice(0, 96);
  return key || 'general';
}

/** 获取或创建登录用户在某张命盘下的唯一会话。 */
async function getOrCreateChatConversation(userId, mode, chartKey, title) {
  if (!userId) return null;
  mode = normalizeChatMode(mode);
  chartKey = normalizeChartKey(chartKey);
  title = String(title || '命理解读').trim().slice(0, 60) || '命理解读';
  const memoryKey = userId + ':' + mode + ':' + chartKey;
  const db = getSupabase();

  if (!db) {
    if (!memStore.chatConversations[memoryKey]) {
      memStore.chatConversations[memoryKey] = {
        id: crypto.randomUUID(), user_id: userId, mode, chart_key: chartKey,
        title, memory_summary: '', created_at: new Date().toISOString(), updated_at: new Date().toISOString()
      };
    }
    return memStore.chatConversations[memoryKey];
  }

  try {
    var query = await db.from('ai_conversations').select('*')
      .eq('user_id', userId).eq('mode', mode).eq('chart_key', chartKey).maybeSingle();
    if (query.error) throw query.error;
    if (query.data) return query.data;

    var row = {
      id: crypto.randomUUID(), user_id: userId, mode, chart_key: chartKey, title,
      created_at: new Date().toISOString(), updated_at: new Date().toISOString()
    };
    var inserted = await db.from('ai_conversations').insert(row).select().single();
    if (inserted.error) {
      // 并发首次提问可能同时创建，冲突后重新读取即可。
      var retry = await db.from('ai_conversations').select('*')
        .eq('user_id', userId).eq('mode', mode).eq('chart_key', chartKey).maybeSingle();
    ...
    console.warn('[chat] 会话表尚未就绪:', e.message);
    return null;
  }
}
...
async function getChatConversation(userId, conversationId) {
  ...
  const db = getSupabase();
  if (!db) {
    return Object.values(memStore.chatConversations).find(function(row) {
      return row.id === conversationId && row.user_id === userId;
    }) || null;
  }
  try {
    var result = await db.from('ai_conversations').select('*')
...
async function getConversationMessages(userId, conversationId, limit) {
  var lim = Math.min(Math.max(parseInt(limit, 10) || 40, 1), 100);
  if (!userId || !conversationId) return [];
  const db = getSupabase();
  if (!db) {
    return memStore.chatHistory.filter(function(row) {
      return row.user_id === userId && row.conversation_id === conversationId;
    }).slice(-lim);
  }
  try {
    var result = await db.from('chat_history').select('role,content,created_at')
      .eq('user_id', userId).eq('conversation_id', conversationId)
      .order('created_at', { ascending: false }).limit(lim);
    if (result.error) throw result.error;
    return (result.data || []).reverse();
  } catch (e) {
    console.warn('[chat] 读取会话历史失败:', e.message);
    return [];
  }
}

async function updateConversationMemory(userId, conversationId, summary) {
  if (!userId || !conversationId) return false;
  summary = String(summary || '').trim().slice(0, 2000);
  const db = getSupabase();
  if (!db) {
    var row = await getChatConversation(userId, conversationId);
    if (!row) return false;
  ...
  try {
    var result = await db.from('ai_conversations')
...
function normalizeCalibrationCandidates(candidates) {
  if (!Array.isArray(candidates)) return [];
  var allowedDomains = ['study', 'career', 'wealth', 'relationship', 'family', 'health', 'change'];
  return candidates.slice(0, 24).map(function(item, index) {
    var year = parseInt(item && item.year, 10);
    var domain = String(item && item.domain || 'change').toLowerCase();
    if (allowedDomains.indexOf(domain) < 0) domain = 'change';
    var prompt = String(item && item.prompt || '').trim().slice(0, 180);
    if (!Number.isInteger(year) || year < 1900 || year > 2200 || !prompt) return null;
    var evidence = Array.isArray(item.evidence) ? item.evidence.slice(0, 5).map(function(value) {
      return String(value || '').trim().slice(0, 160);
    }).filter(Boolean) : [];
    var options = normalizeCalibrationOptions(item && item.options);
    return {
      event_key: String(item.event_key || (year + '-' + domain + '-' + index)).replace(/[^a-zA-Z0-9:_-]/g, '').slice(0, 80),
      event_year: year, domain: domain, prompt: prompt, evidence: evidence, options: options,
      mechanism_key: safeCalibrationKey(item && item.mechanism_key, 80),
...
/** 读取登录用户某张八字命盘的校准记录。 */
async function getChartCalibration(userId, chartKey) {
  if (!userId) return null;
  chartKey = normalizeChartKey(chartKey);
  var memoryKey = userId + ':' + chartKey;
  var db = getSupabase();
  if (!db) return memStore.chartCalibrations[memoryKey] || null;
  try {
    var calibrationResult = await db.from('chart_calibrations').select('*')
      .eq('user_id', userId).eq('chart_key', chartKey).maybeSingle();
    if (calibrationResult.error) throw calibrationResult.error;
    if (!calibrationResult.data) return null;
    var eventsResult = await db.from('chart_calibration_events').select('*')
      .eq('user_id', userId).eq('calibration_id', calibrationResult.data.id)
      .order('event_year', { ascending: false });
    if (eventsResult.error) throw eventsResult.error;
    return { calibration: calibrationResult.data, events: eventsResult.data || [] };
  } catch (error) {
    console.warn('[calibration] 读取失败:', error.message);
    return null;
  }
}

/** 首次生成后锁定候选；后续调用只返回原候选，禁止根据回答重新出题。 */
async function initializeChartCalibration(userId, chartKey, chartSignature, candidates, candidateVersion) {
  if (!userId) return null;
  chartKey = normalizeChartKey(chartKey);
  chartSignature = String(chartSignature || '').replace(/[^甲乙丙丁戊己庚辛壬癸子丑寅卯辰巳午未申酉戌亥男女乾坤|:-]/g, '').slice(0, 80);
  candidates = normalizeCalibrationCandidates(candidates);
  candidateVersion = safeCalibrationKey(candidateVersion, 24) || 'bazi-cal-v3';
  var existing = await getChartCalibration(userId, chartKey);
  if (existing) {
    if (existing.calibration.candidate_version === candidateVersion || !candidates.length) return existing;
    return upgradeChartCalibration(userId, existing, candidates, candidateVersion);
  }
  if (!candidates.length) return null;
  var now = new Date().toISOString();
  var calibration = {
    id: crypto.randomUUID(), user_id: userId, chart_key: chartKey,
    chart_signature: chartSignature, candidate_version: candidateVersion, created_at: now, updated_at: now
  };
  var events = candidates.map(function(item) {
    return Object.assign({
      id: crypto.randomUUID(), calibration_id: calibration.id, user_id: userId,
      answer: null, selected_option: null, selected_detail: null, match_level: null,
      actual_year: null, note: '', answered_at: null, created_at: now, updated_at: now
    }, item);
  });
  var memoryKey = userId + ':' + chartKey;
  var db = getSupabase();
  if (!db) {
    memStore.chartCalibrations[memoryKey] = { calibration: calibration, events: events };
    return memStore.chartCalibrations[memoryKey];
  }
  try {
    var inserted = await db.from('chart_calibrations').insert(calibration).select().single();
    if (inserted.error) {
      // 双窗口同时初始化时保留先写入的一份，绝不覆盖候选。
      return await getChartCalibration(userId, chartKey);
    }
    var eventInsert = await db.from('chart_calibration_events').insert(events).select('*');
    if (eventInsert.error) throw eventInsert.error;
    return { calibration: inserted.data, events: eventInsert.data || events };
  } catch (error) {
    console.warn('[calibration] 初始化失败:', error.message);
    return null;
  }
}

async function upgradeChartCalibration(userId, current, candidates, candidateVersion) {
  var now = new Date().toISOString();
  var answered = (current.events || []).filter(function(event){ return !!event.answer; });
  var unanswered = (current.events || []).filter(function(event){ return !event.answer; });
  var answeredKeys = new Set(answered.map(function(event){ return event.event_key; }));
  var answeredYears = new Set(answered.map(function(event){ return Number(event.event_year); }));
  var availableSlots = Math.max(0, 5 - answered.length);
  var fresh = candidates.filter(function(item){
    return !answeredKeys.has(item.event_key) && !answeredYears.has(Number(item.event_year));
  }).slice(0, availableSlots);
  var memoryKey = userId + ':' + current.calibration.chart_key;
  var db = getSupabase();
  if (!db) {
    var newEvents = fresh.map(function(item) { return Object.assign({
      id:crypto.randomUUID(), calibration_id:current.calibration.id, user_id:userId, answer:null,
      selected_option:null, selected_detail:null, match_level:null, actual_year:null, note:'', answered_at:null,
      created_at:now, updated_at:now
    }, item); });
    current.events = answered.concat(newEvents).sort(function(a,b){return b.event_year-a.event_year});
    current.calibration.candidate_version = candidateVersion;
    current.calibration.updated_at = now;
    memStore.chartCalibrations[memoryKey] = current;
    return current;
  }
  try {
    for (var i = 0; i < fresh.length; i++) {
      var item = fresh[i];
      var same = unanswered.find(function(event){ return event.event_key === item.event_key; });
      if (same) {
        var updated = await db.from('chart_calibration_events').update(Object.assign({}, item, { updated_at:now }))
          .eq('id', same.id).eq('user_id', userId).eq('calibration_id', current.calibration.id);
        if (updated.error) throw updated.error;
      } else {
        var inserted = await db.from('chart_calibration_events').insert(Object.assign({
          id:crypto.randomUUID(), calibration_id:current.calibration.id, user_id:userId, answer:null,
          selected_option:null, selected_detail:null, match_level:null, actual_year:null, note:'', answered_at:null,
          created_at:now, updated_at:now
        }, item));
    ...
    var keepKeys = new Set(fresh.map(function(item){return item.event_key}));
    for (var j = 0; j < unanswered.length; j++) {
      if (keepKeys.has(unanswered[j].event_key)) continue;
      var removed = await db.from('chart_calibration_events').delete().eq('id', unanswered[j].id)
        .eq('user_id', userId).eq('calibration_id', current.calibration.id).is('answer', null);
      if (removed.error) throw removed.error;
    }
    var versioned = await db.from('chart_calibrations').update({ candidate_version:candidateVersion, updated_at:now })
      .eq('id', current.calibration.id).eq('user_id', userId);
    if (versioned.error) throw versioned.error;
    return await getChartCalibration(userId, current.calibration.chart_key);
  } catch (error) {
    console.warn('[calibration] 安全升级失败:', error.message);
    return current;
  }
}

async function answerChartCalibrationEvent(userId, chartKey, eventKey, answer, actualYear, note, selectedOption, selectedDetail, matchLevel) {
  chartKey = normalizeChartKey(chartKey);
  eventKey = String(eventKey || '').replace(/[^a-zA-Z0-9:_-]/g, '').slice(0, 80);
  if (!eventKey) return null;
  var current = await getChartCalibration(userId, chartKey);
  if (!current) return null;
  var target = current.events.find(function(event) { return event.event_key === eventKey; });
  if (!target) return null;
  var normalized = normalizeCalibrationResponse(target, {
    answer: answer, actual_year: actualYear, note: note,
    selected_option: selectedOption, selected_detail: selectedDetail, match_level: matchLevel
  });
  if (normalized.error) return null;
  var patch = Object.assign({}, normalized.value, { answered_at: new Date().toISOString(), updated_at: new Date().toISOString() });
  var db = getSupabase();
  if (!db) { Object.assign(target, patch); current.calibration.updated_at = patch.updated_at; return target; }
  try {
    var result = await db.from('chart_calibration_events').update(patch)
      .eq('id', target.id).eq('user_id', userId).eq('calibration_id', current.calibration.id)
      .select('*').single();
    if (result.error) throw result.error;
    await db.from('chart_calibrations').update({ updated_at: patch.updated_at })
      .eq('id', current.calibration.id).eq('user_id', userId);
    return result.data;
  } catch (error) {
    console.warn('[calibration] 保存回答失败:', error.message);
    return null;
  }
}

async function getChartCalibrationSummary(userId, chartKey) {
  var result = await getChartCalibration(userId, chartKey);
  if (!result) return '';
  var domainNames = { study:'学业', career:'事业', wealth:'财务', relationship:'感情', family:'家庭', health:'身心状态', change:'生活变化' };
  var profile = buildCalibrationProfile(result.events);
  var profileLine = profile.patterns.length ? '【个人应事模型】' + profile.patterns.map(function(pattern) {
    return (domainNames[pattern.domain] || '经历') + '更常落在“' + pattern.label + (pattern.detail ? '－' + pattern.detail : '') + '”';
  }).join('；') + '。这些表现得到不同年份的反馈支持，可优先作为解释线索，但不构成未来事件保证；只调整解释方向，不得改写命盘事实。' : '';
  var tentativeLine = profile.tentativePatterns && profile.tentativePatterns.length ? '【单次校对线索】' + profile.tentativePatterns.map(function(pattern) {
    return (domainNames[pattern.domain] || '经历') + '曾落在“' + pattern.label + (pattern.detail ? '－' + pattern.detail : '') + '”';
  }).join('；') + '。这些仅命中一次，只能作为弱提示，不能概括为用户的稳定规律。' : '';
  var deniedLine = profile.deniedPatterns && profile.deniedPatterns.length ? '【已排除的应事方式】' + profile.deniedPatterns.map(function(item) {
    return (domainNames[item.domain] || '经历') + '的“' + (item.label || item.mechanismKey) + '”〔' + item.mechanismKey + '〕在' + item.years.join('、') + '年被用户明确否认';
  }).join('；') + '。只降低这些具体机制，不要把整个领域一并排除。' : '';
  var eventLines = profile.events.filter(function(event) { return event.answer === 'yes' || event.answer === 'no'; }).slice(0, 20).map(function(event) {
    var year = event.actual_year || event.event_year;
    var picked = optionLabel(event);
    ...
    var rejectedOptions = event.answer === 'no' ? normalizeCalibrationOptions(event.options) : [];
    if(picked&&picked.legacyBroad)state='旧版宽泛题反馈，仅保留原表述，不确认具体事件';
    var statement = picked ? (picked.eventText||picked.label) + (picked.detail ? '，具体是' + picked.detail : '') : (rejectedOptions.length ? '本题全部候选均不符合：' + rejectedOptions.map(function(option) { return option.detail||option.label; }).join('、') : event.prompt);
    var mechanism = picked && picked.mechanismKey ? '；待核对机制=' + picked.mechanismKey : (rejectedOptions.length ? '；已否认候选所属机制=' + rejectedOptions.map(function(option) { return option.mechanism_key || (option.domain + ':general'); }).join(',') : '');
    return year + '年【' + (domainNames[domain] || '经历') + '】' + state + '：' + statement + mechanism + (event.note ? '；用户补充：' + event.note : '');
  });
  return [profileLine, tentativeLine, deniedLine].concat(eventLines).filter(Boolean).join('\n').slice(0, 3000);
}

/** 保存聊天记录（带 user_id；新会话字段不存在时自动降级到旧表结构） */
async function saveUserChatHistory(userId, role, content, meta) {
  const db = getSupabase();
  meta = meta || {};
  var baseRow = {
  ...
  var extendedRow = Object.assign({}, baseRow);
  if (meta.conversationId) extendedRow.conversation_id = meta.conversationId;
  if (meta.mode) extendedRow.mode = normalizeChatMode(meta.mode);
  if (meta.chartKey) extendedRow.chart_key = normalizeChartKey(meta.chartKey);

  if (!db) {
    memStore.chatHistory.push(extendedRow);
    if (memStore.chatHistory.length > 500) memStore.chatHistory.splice(0, 100);
    return;
  }
  try {
    var result = await db.from('chat_history').insert(extendedRow);
    if (result.error && meta.conversationId) {
      // 服务器先更新、数据库迁移稍后执行时，仍保留旧版聊天记录能力。
      await db.from('chat_history').insert(baseRow);
    }
  } catch (e) { /* 聊天记录失败不得影响回答与扣费 */ }
}
