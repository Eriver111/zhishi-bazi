...
/** 记录免费次数使用 */
async function trackFreeUsage(identifier) {
  ...
  if (db) {
    ...
    if (data) {
      ...
      await db.from('free_credits_log').insert({ identifier, used_count: 1, created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
    }
    return { used: newCount, remaining: Math.max(0, maxFree - newCount) };
  }
  if (!memStore.freeLog[identifier]) memStore.freeLog[identifier] = 0;
  memStore.freeLog[identifier]++;
  const used = memStore.freeLog[identifier];
