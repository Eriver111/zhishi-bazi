...
async function initializeChartCalibration(userId, chartKey, chartSignature, candidates) {
  if (!userId) return null;
  chartKey = normalizeChartKey(chartKey);
  ...
  var calibration = {
    ...
    chart_signature: chartSignature, candidate_version: 'bazi-cal-v2', created_at: now, updated_at: now
  };
...
async function getChartCalibrationSummary(userId, chartKey) {
  ...
  return [profileLine].concat(eventLines).filter(Boolean).join('\n');
}
