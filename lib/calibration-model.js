...
function buildCalibrationProfile(events) {
  ...
  (events || []).forEach(function(event) {
    if (event.answer === 'yes') {
      ...
      deniedItems.forEach(function(item) {
        ...
        const key = domain + ':' + mechanism;
        if (seen.has(key)) return;
        seen.add(key);
        denied[domain] = (denied[domain] || 0) + 1;
        if (!deniedMechanisms[key]) deniedMechanisms[key] = { domain, mechanismKey: mechanism, label: item.label || '', count: 0, years: [] };
  ...
  // 一次命中只作为初步线索；至少跨两个年份重复命中，才升级为稳定个人模型。
  ...
  return { version: 'bazi-cal-v3', patterns, tentativePatterns, denied, deniedPatterns: Object.values(deniedMechanisms) };
}
