...
function buildCalibrationProfile(events) {
  ...
  (events || []).forEach(function(event) {
    if (event.answer === 'yes') {
      ...
      const mechanism = safeKey(event.mechanism_key, 80) || (domain + ':general');
      const key = domain + ':' + mechanism;
      if (!deniedMechanisms[key]) deniedMechanisms[key] = { domain, mechanismKey: mechanism, count: 0, years: [] };
  ...
  return { version: 'bazi-cal-v3', patterns, denied, deniedPatterns: Object.values(deniedMechanisms) };
}
