...
async function sendCode(toEmail, code) {
  try {
    var result = await $dm.singleSendMail(new DM.SingleSendMailRequest({
    ...
    console.log('DM send OK:', toEmail, result.body);
    return result;
  } catch (e) {
    console.error('DM API Error:', JSON.stringify(e, null, 2));
    throw e;
  }
}
