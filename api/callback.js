...
module.exports = async function handler(req, res) {
  try {
    ...
    if (outTradeNo.startsWith('credit3_')) {
      await doInsertWithRetry(code, outTradeNo, 3, userId, channel);
    } else if (outTradeNo.startsWith('credit10_')) {
      await doInsertWithRetry(code, outTradeNo, 10, userId, channel);
    } else if (outTradeNo.startsWith('credit20_')) {
      await doInsertWithRetry(code, outTradeNo, 20, userId, channel);
    } else if (outTradeNo.startsWith('monthly_')) {
      await doMonthlyWithRetry(code, outTradeNo, 30, userId, channel);
    } else if (outTradeNo.startsWith('credit_')) {
      await doInsertWithRetry(code, outTradeNo, 10, userId, channel);
    } else if (outTradeNo.startsWith('aichat_')) {
      await doInsertWithRetry(code, outTradeNo, 5, userId, channel);
    } else {
