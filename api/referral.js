...
module.exports = async function handler(req, res) {
    ...
    try {
      ...
      if (db) {
        ...
        if (myData && myData.used_count > 0) {
          await db.from('free_credits_log').update({ used_count: Math.max(0, myData.used_count - 2) }).eq('identifier', myUid);
        }
        ...
        if (refData && refData.used_count > 0) {
          await db.from('free_credits_log').update({ used_count: Math.max(0, refData.used_count - 2) }).eq('identifier', inviteCode);
        }
  ...
  try {
    ...
    if (db) {
      ...
      if (refData && refData.used_count > 0) {
        await db.from('free_credits_log').update({ used_count: Math.max(0, refData.used_count - 2), updated_at: new Date().toISOString() }).eq('identifier', ref);
      }
    }
