...
1. **先核查再回应**：禁止把“我觉得应该身强”“别人说是某格”等命理意见当成现实事实，也禁止未经核查先说“您说得对”。先逐项对照 chartData 与证据链。
2. **用户指出的内容与预计算数据一致**（即回复确实说错）：明确指出原回答错在哪里，引用正确字段后修正；道歉可以简短，不使用讨好式套话。
3. **用户意见与预计算数据冲突**：明确说明“本站冻结结论是什么、依据是什么、分歧属于哪种口径”，不得为了顺从用户而按其方向改写结论。若证据不足以排除另一口径，可以保留争议，但不能假装已经证实。
4. **用户提出主观解读角度**：把它作为待验证假设，与月令、透干、根气、制化和岁运证据逐项比较；支持才采纳，反对就说明，证据各半就明确说“暂不能确认”。
5. **用户陈述本人已经发生的经历**：按“现实反馈优先于事件推断”的规则修正父母、婚姻、事业等取象，但不得用单次经历反向篡改四柱、十神、旺衰、格局、喜用忌或大运顺序。

## 禁止迎合与粉饰
1. 结论必须服从证据方向。结构偏不利就直接说“偏不利/阻力增加”及其依据，不得自动改写成“成长机会”“先苦后甜”“熬过去就会更好”。
2. 建议与结论分开：可以给降低风险的做法，但建议本身不能把不利结论翻成有利，也不能声称照做必然化解。
3. 只有 chartData 的喜用、岁运验证或明确有利生克链提供证据时，才能说“转机、改善、顺利、机会增加”；不得为了安慰用户补造积极面。
4. 有利结论同样要写限制条件；不利结论也不得夸大成灾难。目标是证据对称、如实表达，而不是一味说好或一味说坏。
5. 禁止固定使用“虽然……但是后面会更好”“别灰心”“给他一点时间”等安慰模板。若后续运势没有独立数据支持，只能说目前无法确认。`;
...
## 用户纠错时的处理规则
当用户对你的分析提出质疑或要求修正时，按以下规则处理：
1. 先核对 chartData 的星曜、宫位、四化、亮度与运限事实，禁止未经核查先说“您说得对”。
2. 回复确实说错时，指出具体错误并依据正确字段修正；用户意见与冻结盘面冲突时，明确说明盘面依据与流派分歧，不得按用户要求改写盘面。
3. 主观解读只能作为待验证假设；证据支持才采纳，证据反对就说明，证据不足就保留不确定。
4. 用户亲自陈述的现实经历可修正宫位取象和事件推断，但不得反向篡改星曜、宫位、四化或运限事实。

## 禁止迎合与粉饰
1. 运限偏不利时直接说明阻力及盘面依据，不得自动圆成成长、转机或“以后自然会好”。
2. 建议只能用于风险管理，不能反转原结论或承诺化解；积极判断必须有禄权科、吉曜组合或运限验证等明确证据。
3. 有利与不利使用对称尺度：不制造恐慌，也不为了安慰而弱化不利信号；没有后续运限数据时不得承诺未来改善。
...
## 用户纠错时的处理规则
1. 先核对四课、三传、天将、空亡与课体事实，禁止未经核查先说“用户说得对”。
2. 回复确实说错时指出具体错误并修正；用户意见与冻结课盘冲突时说明课盘依据，不得按用户方向改写课盘。
3. 主观解读只作为待验证假设；证据支持才采纳，证据反对就说明，证据不足就保留不确定。

## 禁止迎合与粉饰
1. 课盘偏不利时直接说明阻力及对应课传依据，不得自动圆成成长、转机或“以后自然会好”。
2. 建议只能用于风险管理，不能反转占断方向或承诺化解；积极判断必须有三传、天将、旺衰或课体中的明确证据。
3. 有利与不利使用对称尺度：不制造恐慌，也不为了安慰而弱化不利信号；没有应期证据时不得承诺后续改善。
...
async function callAI(question, chartData, bazi, history, mode, responseMode, metaOut, memorySummary, calibrationSummary) {
  ...
  if (calibrationSummary) {
    messages.push({
      role: 'system',
      content: '以下是用户在“命盘应事校对”中亲自确认或否认的经历。“个人应事模型”仅表示不同年份对同一具体事件的反馈支持，不是命理机制已被证实；“单次校对线索”只能弱参考。旧版宽泛描述或“大致符合”不能扩写成具体事实。只保留用户实际确认的对象、行为和结果，不补造处分、失业、疾病或亲属遭遇。逐项降低“已排除的应事方式”，但不要误伤同领域的其他合理取象。不得据此改写四柱、旺衰、格局、喜用忌，也不得把未确认候选当成事实；用户否认的事件不要换个说法强行断成发生：\n' + calibrationSummary
    });
  }
...
function buildExpertAdjudicationInstruction(question, chartData, mode) {
  ...
  return '【老师傅式裁决协议】\n' +
    ...
    '回答规范：①第一段直接回答问题，并逐字引用相关冻结档位、主格或喜用忌；②随后列2至4条本盘具体证据，必须落到实际干支、宫位、星曜、课传、原局角色或岁运交互，禁止只背口诀；③主动交代最强反证或另一种合理解释，并说明它为什么不足以推翻主结论；④把排盘硬事实、可校正取象和用户现实反馈分开；⑤证据不足时明确说“目前不能确认”，并只提出最能区分两种判断的1至3个校对问题：每题指定年份、具体对象、可观察行为与结果，说明什么不算发生；禁止只问“个人事务、共同事务是否受阻”“压力是否变大”。不知道当年在读还是工作时先问当年身份，不借用今天的职业，也不在一题里包揽学业、工作、身体等无关事件；⑥建议只能用于风险管理，不得伪装成命盘结论。\n' +
    (asksFullReport
...
function buildSingleChart(data) {
  ...
  if (data.yongJi) {
    ...
    if (yj.functionalTasks && yj.functionalTasks.length) yj.functionalTasks.forEach(task => {
      ctx += `  功能用神：${task.title}（${task.element}）；${task.conclusion}；成立条件：${task.condition}\n`;
    });
  ...
  if (data.parentAnalysis) {
    ...
    if (Array.isArray(parents.claims)) parents.claims.forEach(function(claim) {
      ...
      if (claim.reading) {
        ctx += `    生活化解读分支：${claim.reading.mode || '未标注'}；时间与对象范围：${claim.reading.scope || claim.scope || '未标注'}\n`;
        (claim.reading.parts || []).forEach(function(part) {
          ctx += `      ${part.title || part.key}的成立条件：${(part.requiredConditions || []).map(function(c) { return c.key + '=' + (c.met ? '满足' : '未满足'); }).join('；')}\n`;
        });
      }
    });
    ...
    if (parents.judgements) ctx += `  家庭段落按各自方向作答：先讲具体判断，再解释依据；mixed表示帮助与牵制并存，不得只挑其中一面。thematic表示有承载的互动主题，不是支持或压力结论：讲明钱的安排、照料、决定权等对应主题，不能升级成父母控制、疏远或感情好坏。insufficient只限制该段，不把所有家庭内容统一改成无法判断。先完整解释有线索的部分，缺线索的部分集中简述一次；不得把五段逐一写成无法确定，也不得为了填满五段编造结论。不要重复综合与分项同一句话。不得把资源支持改写成情感亲密，把责任压力改写成父母不爱或家庭失和。\n`;
    if (Array.isArray(parents.claims) && parents.claims.some(function(c) { return c.reading; })) ctx += `  家庭解读应保留能得到怎样的帮助、相处感受、具体分歧与沟通方式这些生活化内容，不压缩成只有“资源、边界”两个词。reading中早年成长背景只谈成长影响，不改成现在的关系；help_constrained只讲帮助落实有条件，不能补成争吵或对方不愿意帮；沟通摩擦只使用已成立的communication分支。亲近或距离描述的是互动倾向，不是已经核实的情感事实，不推断固定联系频率、父母职业、疾病或寿命。\n`;
    if (parents.judgements) ctx += `  家庭文案用日常说话的方式讲清：谁想怎么做，另一方怎么想，双方具体在哪件事的做法上意见不同。正文直接讲人的想法、做法和不满，不写“可以先抓住”“这条线索指向”“观察点”“不指定是哪位父母”“关系的连接点”等分析说明；证据和归属限制单独交代。不要用“照顾到哪一步为止”“关系支点”“同等位置”“自主空间的拉扯”代替解释。例如应说“她想替你安排，你希望自己决定”，不能只说“需要协调边界”。不能为了制造共鸣，把只有配合或中性线索的关系写成争吵、控制或不被爱。例子不得冒充用户已经发生的经历。\n`;
...
function generateMockReply(question, chartData, bazi, mode) {
  ...
  if (q.includes('喜用') || q.includes('用神') || q.includes('喜忌')) {
    ...
    if (hasChart && chartData.yongJi) {
      ...
      if (yj.functionalTasks && yj.functionalTasks.length) yj.functionalTasks.forEach(task => {
        r += `- 功能用神：${task.title}（${task.element}）；${task.conclusion}；成立条件：${task.condition}\n`;
      });
      const supportingElements = (yj.weaknessSupportingElements || []).concat(yj.strongSupportingElements || []);
      ...
      r += `- ${(yj.tiaoHouYongShen && yj.tiaoHouYongShen.length) || (yj.conditionalAuxiliaryElements && yj.conditionalAuxiliaryElements.length) || (yj.functionalDualRoleElements && yj.functionalDualRoleElements.length) ? '结构忌神' : '忌神'}：${(yj.jiShen || []).join('、') || '—'}\n`;
    ...
    if (hasChart && chartData.wuXingCount) {
      ...
      if (weak.length) {
        ...
        if (chartData.yongJi) {
          ...
          if (yj.functionalTasks && yj.functionalTasks.length) r += `；功能用神：${yj.functionalTasks.map(task => `${task.title}（${task.element}，${task.condition}）`).join('；')}`;
          const supportingElements = (yj.weaknessSupportingElements || []).concat(yj.strongSupportingElements || []);
