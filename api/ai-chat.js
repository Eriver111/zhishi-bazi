...
- **currentDaYun/currentLiuNian**（当前大运流年）：已精确计算的当前运势周期，回答运势类问题以此为据
...
- 每段不超过 3 句话，用短句
- 最后一定加上一句「这只是传统文化的角度，别太往心里去，日子还是你自己过出来的」
...
async function callAI(question, chartData, bazi, history, mode) {
  ...
  // 模式指令
  if (mode === 'simple') {
    messages.push({ role: 'system', content: '本轮使用**白话模式**。用日常口语回答，不引经典原文，术语后附括号解释，每段不超过3句，最后说"这只是传统文化的角度，别太往心里去，日子还是你自己过出来的"。' });
  } else if (mode === 'pro') {
