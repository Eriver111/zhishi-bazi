// PAT04 表达专项单盘重跑（2026-08-14）：回归批中 PAT04 因 TH07 socket hangup 双扣积分致额度耗尽而失败。
// 本脚本复制自 _aireport_batch.js，仅跑 PAT04（丁亥 己酉 甲辰 庚午），额度给 2 防 hangup 双扣。
// 验证点：火忌表达是否被新 prompt（事实锁 6 条）过度纠正。汇总写 00-summary-pat04.md 不覆盖 13 盘汇总。
// 流程同回归批：字节验证 → 反查出生 → 插码 AISMOKE04 → 真实 POST /api/ai-chat（mode=pro, qa_debug）→ 验证 DB → 删码。
...
(async function () {
  ...
  var { error: insErr } = await db.from('user_credits').upsert({
    code: TEST_CODE, order_id: 'qa-report-batch-20260814', credits: 2, total_used: 0,
  ...
  // ---- 汇总 ----
  var sum = '# PAT04 表达专项单盘重跑汇总（2026-08-14）\n\n';
  sum += '> 回归批中 PAT04 因 TH07 socket hangup 双扣积分致额度耗尽而失败，本脚本单盘补跑。\n';
  sum += '> 线上真实调用 zhishi.online /api/ai-chat（生产 DeepSeek），mode=pro，新会话首问（history=[]），qa_debug 透出 V1 validator warnings。\n';
  sum += '> 测试兑换码 AISMOKE04（qa 专用，额度 2 防 hangup 双扣），调用完成后已删除；chat_history 证据行保留。\n';
  sum += '> 验证点：火忌表达是否被新 prompt（事实锁 6 条）过度纠正。\n\n';
  ...
  fs.writeFileSync(path.join(OUT_DIR, '00-summary-pat04.md'), sum, 'utf8');
  console.log('\n✅ 汇总已落盘 ' + OUT_DIR + '/00-summary-pat04.md');
})().catch(function (e) {
